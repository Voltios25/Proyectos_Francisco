...
function listar() {
  tabla = $('#tbllistado').dataTable({
    ...
    "ajax": {
      url: '../controlador/personal.php?op=listar',
      type: "get",
      dataType: "json",
      error: function(e) {
...
function guardaryeditar(e) {
  ...
  $.ajax({
    url: "../controlador/personal.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function mostrar(id_conductor) {
  ...
  $.post("../controlador/personal.php?op=mostrar", {
...
function activar(id_conductor) {
  bootbox.confirm("¿ESTÁ SEGURO DE ACTIVAR ESTE REGISTRO?", function(result) {
    if (result) {
      $.post("../controlador/personal.php?op=activar", {
        id_conductor: id_conductor
      }, function(e) {
        bootbox.alert(e);
        tabla.ajax.reload();
